...
other_train_image = torch.cat((CN_train_image, EMCI_train_image, LMCI_train_image), 0)
print(other_train_image.shape)

other_val_image = torch.cat((CN_val_image, EMCI_val_image, LMCI_val_image), 0)
print(other_val_image.shape)

other_test_image = torch.cat((CN_test_image, EMCI_test_image, LMCI_test_image), 0)
...
train_image = torch.cat((AD_train_image, other_train_image), 0)
print(train_image.shape)

AD_train_label = torch.zeros(AD_train_image.shape[0])
other_train_label = torch.ones(other_train_image.shape[0])

train_label = torch.cat((AD_train_label, other_train_label))
...
val_image = torch.cat((AD_val_image, other_val_image), 0)
print(val_image.shape)

AD_val_label = torch.zeros(AD_val_image.shape[0])
other_val_label = torch.ones(other_val_image.shape[0])

val_label = torch.cat((AD_val_label, other_val_label))
...
test_image = torch.cat((AD_test_image, other_test_image), 0)
print(test_image.shape)

AD_test_label = torch.zeros(AD_test_image.shape[0])
other_test_label = torch.ones(other_test_image.shape[0])

test_label = torch.cat((AD_test_label, other_test_label))
...
if __name__ == '__main__':
    criterion = FocalLoss(weight=torch.FloatTensor([5.184347826086957, 1.238985868661679]).to(device))
    ...
    for epoch in range(num_epochs):
        train_loss = 0.0
        train_acc = 0.0
        PATH = './onevsall/300epochs/weight_ad_others_learningcurves.pkl'
...
#         scheduler.step()
        ...
        if epoch == 149:  
            torch.save(model.state_dict(), './onevsall/300epochs/weight_ad_others_learningcurves_150.pkl')
...
pd.DataFrame(history).to_csv("./onevsall/300epochs/history_ad_others_learningcurves.csv")
...
df_cm = pd.DataFrame(confmat, index = [i for i in ["AD", "others"]],
                     columns = [i for i in ["AD", "others"]])
