...
other_train_image = torch.cat((CN_train_image, EMCI_train_image, AD_train_image), 0)
print(other_train_image.shape)

other_val_image = torch.cat((CN_val_image, EMCI_val_image, AD_val_image), 0)
print(other_val_image.shape)

other_test_image = torch.cat((CN_test_image, EMCI_test_image, AD_test_image), 0)
...
train_image = torch.cat((LMCI_train_image, other_train_image), 0)
...
val_image = torch.cat((LMCI_val_image, other_val_image), 0)
...
test_image = torch.cat((LMCI_test_image, other_test_image), 0)
...
if __name__ == '__main__':
    criterion = FocalLoss(weight=torch.FloatTensor([7.397022332506204, 1.156322730799069]).to(device))
    ...
    for epoch in range(num_epochs):
        train_loss = 0.0
        train_acc = 0.0
        PATH = './onevsall/300epochs/weight_lmci_others_learningcurves.pkl'
...
#         scheduler.step()
        ...
        if epoch == 149:  
            torch.save(model.state_dict(), './onevsall/300epochs/weight_lmci_others_learningcurves_150.pkl')
...
pd.DataFrame(history).to_csv("./onevsall/300epochs/history_lmci_others_learningcurves.csv")
...
df_cm = pd.DataFrame(confmat, index = [i for i in ["LMCI", "others"]],
                     columns = [i for i in ["LMCI", "others"]])
