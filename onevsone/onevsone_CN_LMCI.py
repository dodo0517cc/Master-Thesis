...
# CN_external_image = CN_image[idx3:]
# print(CN_external_image.shape)
...
# LMCI_external_image = LMCI_image[idx3:]
# print(LMCI_external_image.shape)
...
train_image = torch.cat((CN_train_image, LMCI_train_image), 0)
print(train_image.shape)

CN_train_label = torch.zeros(CN_train_image.shape[0])
other_train_label = torch.ones(LMCI_train_image.shape[0])
...
val_image = torch.cat((CN_val_image, LMCI_val_image), 0)
print(val_image.shape)

CN_val_label = torch.zeros(CN_val_image.shape[0])
other_val_label = torch.ones(LMCI_val_image.shape[0])
...
test_image = torch.cat((CN_test_image, LMCI_test_image), 0)
print(test_image.shape)

CN_test_label = torch.zeros(CN_test_image.shape[0])
other_test_label = torch.ones(LMCI_test_image.shape[0])
...
batch_size = 16
train_loader = DataLoader(TensorDataset(train_image.float(), train_label.long()), batch_size=batch_size, sampler=train_sampler)
...
#     criterion = nn.CrossEntropyLoss(weight=torch.FloatTensor([1.391304347826087, 3.5555555555555554]).to(device))
    criterion = FocalLoss(weight=torch.FloatTensor([1.391304347826087, 3.5555555555555554]).to(device))
    ...
    for epoch in range(num_epochs):
        train_loss = 0.0
        train_acc = 0.0
        PATH = './onevsone/300epochs/weight_cn_lmci.pkl'
        tp_train = []
...
#         scheduler.step()
        ...
        if epoch == 149:  
            torch.save(model.state_dict(), './onevsone/300epochs/weight_2class_cn_lmci_150.pkl')
...
pd.DataFrame(history).to_csv("./onevsone/300epochs/history_cn_lmci.csv")
...
df_cm = pd.DataFrame(confmat, index = [i for i in ["CN", "LMCI"]],
                     columns = [i for i in ["CN", "LMCI"]])
