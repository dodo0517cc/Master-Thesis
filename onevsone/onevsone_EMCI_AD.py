...
train_image = torch.cat((EMCI_train_image, AD_train_image), 0)
...
val_image = torch.cat((EMCI_val_image, AD_val_image), 0)
...
test_image = torch.cat((EMCI_test_image, AD_test_image), 0)
...
if __name__ == '__main__':
    criterion = FocalLoss(weight=torch.FloatTensor([1.7428940568475453, 2.3460869565217393]).to(device))
    ...
    for epoch in range(num_epochs):
        train_loss = 0.0
        train_acc = 0.0
        PATH = './onevsone/300epochs/weight_emci_ad.pkl'
        tp_train = []
...
#         scheduler.step()
        ...
        if epoch == 149:  
            torch.save(model.state_dict(), './onevsone/300epochs/weight_2class_emci_ad_150.pkl')
        if (epoch+1) % 1 == 0:
...
pd.DataFrame(history).to_csv("./onevsone/300epochs/history_emci_ad.csv")
...
df_cm = pd.DataFrame(confmat, index = [i for i in ["EMCI", "AD"]],
                     columns = [i for i in ["EMCI", "AD"]])
