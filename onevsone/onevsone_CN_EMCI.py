...
train_image = torch.cat((CN_train_image, EMCI_train_image), 0)
print(train_image.shape)

CN_train_label = torch.zeros(CN_train_image.shape[0])
other_train_label = torch.ones(EMCI_train_image.shape[0])
...
val_image = torch.cat((CN_val_image, EMCI_val_image), 0)
print(val_image.shape)

CN_val_label = torch.zeros(CN_val_image.shape[0])
other_val_label = torch.ones(EMCI_val_image.shape[0])
...
test_image = torch.cat((CN_test_image, EMCI_test_image), 0)
print(test_image.shape)

CN_test_label = torch.zeros(CN_test_image.shape[0])
other_test_label = torch.ones(EMCI_test_image.shape[0])
...
if __name__ == '__main__':
    criterion = nn.CrossEntropyLoss(torch.FloatTensor([1.6297803091944671, 2.5878552971576227]).to(device))
    ...
    for epoch in range(num_epochs):
        train_loss = 0.0
        train_acc = 0.0
        PATH = './onevsone/300epochs/weight_2class_cn_emci.pkl'
        tp_train = []
...
#         scheduler.step()
        ...
        if epoch == 149:  
            torch.save(model.state_dict(), './onevsone/300epochs/weight_2class_cn_emci_150.pkl')
...
pd.DataFrame(history).to_csv("./onevsone/300epochs/history_cn_emci.csv")
...
df_cm = pd.DataFrame(confmat, index = [i for i in ["CN", "EMCI"]],
                     columns = [i for i in ["CN", "EMCI"]])
