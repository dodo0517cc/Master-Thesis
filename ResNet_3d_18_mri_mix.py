...
#         self.train = train
    def __getitem__(self,index):
        mix = self.df.loc[:,["mix"]].iloc[index].values
        onehot_mix = [0] * 13
        onehot_mix[int(mix)] = 1
        mix_tensor = torch.FloatTensor(onehot_mix)
...
#         Y = torch.tensor(Y)
        return mix_tensor, image_tensor, label_tensor
    def __len__(self):
...
class Classifier_model(nn.Module):
    def __init__(self):
        ...
        self.fc_0 =  nn.Sequential(nn.Linear(num_ftrs, 64),
                                    nn.BatchNorm1d(64),
                                    nn.SELU(),
                                    nn.Dropout(0.2),
                                    nn.Linear(64, 4) 
                                    )
        self.fc_1 =  nn.Sequential(nn.Linear(num_ftrs, 64),
                            nn.BatchNorm1d(64),
                            nn.SELU(),
                            nn.Dropout(0.2),
                            nn.Linear(64, 4) 
                            )
        self.fc_2 =  nn.Sequential(nn.Linear(num_ftrs, 64),
                            nn.BatchNorm1d(64),
                            nn.SELU(),
                            nn.Dropout(0.2),
                            nn.Linear(64, 4) 
                            )
        self.fc_3 =  nn.Sequential(nn.Linear(num_ftrs, 64),
                                    nn.BatchNorm1d(64),
                                    nn.SELU(),
                                    nn.Dropout(0.2),
                                    nn.Linear(64, 4) 
                                    )
        self.fc_4 =  nn.Sequential(nn.Linear(num_ftrs, 64),
                            nn.BatchNorm1d(64),
                            nn.SELU(),
                            nn.Dropout(0.2),
                            nn.Linear(64, 4) 
                            )
        self.fc_5 =  nn.Sequential(nn.Linear(num_ftrs, 64),
                            nn.BatchNorm1d(64),
                            nn.SELU(),
                            nn.Dropout(0.2),
                            nn.Linear(64, 4) 
                            )
        self.fc_6 =  nn.Sequential(nn.Linear(num_ftrs, 64),
                                    nn.BatchNorm1d(64),
                                    nn.SELU(),
                                    nn.Dropout(0.2),
                                    nn.Linear(64, 4) 
                                    )
        self.fc_7 =  nn.Sequential(nn.Linear(num_ftrs, 64),
                            nn.BatchNorm1d(64),
                            nn.SELU(),
                            nn.Dropout(0.2),
                            nn.Linear(64, 4) 
                            )
        self.fc_8 =  nn.Sequential(nn.Linear(num_ftrs, 64),
                            nn.BatchNorm1d(64),
                            nn.SELU(),
                            nn.Dropout(0.2),
                            nn.Linear(64, 4) 
                            )
        self.fc_9 =  nn.Sequential(nn.Linear(num_ftrs, 64),
                                    nn.BatchNorm1d(64),
                                    nn.SELU(),
                                    nn.Dropout(0.2),
                                    nn.Linear(64, 4) 
                                    )
        self.fc_10 =  nn.Sequential(nn.Linear(num_ftrs, 64),
                            nn.BatchNorm1d(64),
                            nn.SELU(),
                            nn.Dropout(0.2),
                            nn.Linear(64, 4) 
                            )
        self.fc_11 =  nn.Sequential(nn.Linear(num_ftrs, 64),
                            nn.BatchNorm1d(64),
                            nn.SELU(),
                            nn.Dropout(0.2),
                            nn.Linear(64, 4) 
                            )
    def forward(self, x, mix): # text = [batch size,sent_length]
  
        logits = x
        mix_0 = mix[:,0].reshape(mix.shape[0], 1)
        mix_1 = mix[:,1].reshape(mix.shape[0], 1)
        mix_2 = mix[:,2].reshape(mix.shape[0], 1)
        mix_3 = mix[:,3].reshape(mix.shape[0], 1)
        mix_4 = mix[:,4].reshape(mix.shape[0], 1)
        mix_5 = mix[:,5].reshape(mix.shape[0], 1)
        mix_6 = mix[:,6].reshape(mix.shape[0], 1)
        mix_7 = mix[:,7].reshape(mix.shape[0], 1)
        mix_8 = mix[:,8].reshape(mix.shape[0], 1)
        mix_9 = mix[:,9].reshape(mix.shape[0], 1)
        mix_10 = mix[:,10].reshape(mix.shape[0], 1)
        mix_11 = mix[:,11].reshape(mix.shape[0], 1)
        logits = self.Flatten(logits)
        logits = mix_0 * self.fc_0(logits) + mix_1 * self.fc_1(logits) + mix_2 * self.fc_2(logits) +  mix_3 * self.fc_3(logits) + mix_4 * self.fc_4(logits) + mix_5 * self.fc_5(logits) +mix_6 * self.fc_6(logits) + mix_7 * self.fc_7(logits) + mix_8 * self.fc_8(logits) + mix_9 * self.fc_9(logits) + mix_10 * self.fc_10(logits) + mix_11 * self.fc_11(logits)
        return logits
...
#     criterion = nn.CrossEntropyLoss()
    ...
    PATH = './mmse/300epochs/weight_mix_learningcurves_300.pkl'
    model.load_state_dict(torch.load('./mmse/300epochs/weight_apoe_learningcurves_300.pkl'))
    ...
    #     scheduler = optim.lr_scheduler.StepLR(optimizer=optimizer, step_size=5, gamma=0.9)
        model.train()
        classifier.train()
        for i, (mix, data, label) in enumerate(train_loader):
          ...
          outputs = classifier(outputs.to(device), mix.to(device)) 
        ...
        with torch.no_grad():
          val_loss = 0.0
          val_acc = 0.0
          for i, (mix, data, label) in enumerate(val_loader):
        ...
        if val_acc/len(val_image) > max_acc:
            ...
            torch.save(classifier.state_dict(), './mmse/weight_mix_learningcurves_classifier_300.pkl')
        ...
        if epoch == 149:  
            torch.save(model.state_dict(), './mmse/weight_mix_learningcurves_150.pkl')
            torch.save(classifier.state_dict(), './mmse/weight_mix_learningcurves_classifier_150.pkl')
...
pd.DataFrame(history).to_csv("./mmse/300epochs/history_mix_learningcurves.csv")
...
plt.plot(train_acc_resnet18_pre[:300], label='train_acc')
plt.plot(val_acc_resnet18_pre[:300], label='val_acc')
plt.plot(np.array(loss_resnet18_pre[:300])*10, label='train_loss')
...
classifier.load_state_dict(torch.load('./mmse/weight_mix_learningcurves_classifier_300.pkl'))
...
model.eval()
classifier.eval()
for i, (mix, data, label) in enumerate(test_loader):
