...
other_train_image = torch.cat((EMCI_train_image, LMCI_train_image), 0)
print(other_train_image.shape)

other_val_image = torch.cat((EMCI_val_image, LMCI_val_image), 0)
print(other_val_image.shape)

other_test_image = torch.cat((EMCI_test_image, LMCI_test_image), 0)
...
train_image = torch.cat((other_train_image, AD_train_image), 0)
print(train_image.shape)

other_train_label = torch.zeros(other_train_image.shape[0])
AD_train_label = torch.ones(AD_train_image.shape[0])

train_label = torch.cat((other_train_label, AD_train_label))
...
val_image = torch.cat((other_val_image, AD_val_image), 0)
print(val_image.shape)

other_val_label = torch.zeros(other_val_image.shape[0])
AD_val_label = torch.ones(AD_val_image.shape[0])

val_label = torch.cat((other_val_label, AD_val_label))
...
test_image = torch.cat((other_test_image, AD_test_image), 0)
print(test_image.shape)

other_test_label = torch.zeros(other_test_image.shape[0])
AD_test_label = torch.ones(AD_test_image.shape[0])

test_label = torch.cat((other_test_label, AD_test_label))
...
if __name__ == '__main__':
    criterion = FocalLoss(torch.FloatTensor([1.4885301614273576, 3.0469565217391303]).to(device))
    model = CNNRNN_model().to(device)
#     model.load_state_dict(torch.load('./weights/hierarchical/weight_mci_ad_learningcurves.pkl'))
    ...
    for epoch in range(num_epochs):
        train_loss = 0.0
        train_acc = 0.0
        PATH = './weights/hierarchical/weight_mci_ad_learningcurves.pkl'
...
#         scheduler.step()
        ...
        if epoch == 149:  
            torch.save(model.state_dict(), './weights/hierarchical/weight_mci_ad_learningcurves_150.pkl')        
...
pd.DataFrame(history).to_csv("./weights/hierarchical/history_mci_ad_learningcurves.csv")
...
df_cm = pd.DataFrame(confmat, index = [i for i in ["MCI", "AD"]],
                     columns = [i for i in ["MCI", "AD"]])
...
external_idx = np.load("./weights/hierarchical/cn_others_copy1.npy")
...
external_image = torch.cat((CN_test_image, other_test_image, AD_test_image), 0)
...
np.save("./weights/hierarchical/mci_ad_copy1.npy", tp_test)


# In[59]:


np.save("./weights/hierarchical/ad_external_copy1.npy", ad)
