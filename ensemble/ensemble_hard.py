...
# other_image = torch.cat((AD_image, EMCI_image, LMCI_image), 0)
...
cn = np.load("./weights/hierarchical/300epochs/cn_external_copy1.npy")
emci = np.load("./weights/hierarchical/300epochs/emci_external_copy1.npy")
lmci = np.load("./weights/hierarchical/300epochs/lmci_external_copy1.npy")
ad = np.load("./weights/hierarchical/300epochs/ad_external_copy1.npy")
print(len(cn))
print(len(emci))
print(len(lmci))
print(len(ad))
...
hierarchical = hierarchical_model(cn, emci, lmci, ad)


# In[31]:


hierarchical_test = np.eye(4)[hierarchical]
...
def training(loader, train=True):
    for epoch in range(300):
        pred_label = []
        for i, (idx, data, label) in enumerate(loader):  
              # forward
            if train == True:
                data = augmentation(data)  
            data = dim1_to_dim3(data)
            scores = onevsone_model(data)
            scores = torch.tensor(scores)
            if i == 0:
                print(idx)
                stacked_scores = scores
            if i > 0:
                stacked_scores = np.vstack((stacked_scores, scores))

            predict = np.argmax(scores.cpu().data.numpy(), axis=1) 
            for i in predict:
                pred_label.append(i)
            stacked_hard = torch.tensor(np.eye(4)[pred_label]) 
        if epoch == 0:
            save_scores = stacked_scores
            save_hard = stacked_hard
        if epoch > 0:
            save_scores = np.vstack((save_scores, stacked_scores))
            save_hard = np.vstack((save_hard, stacked_hard))
    return save_scores, save_hard


# In[ ]:


save_scores, save_hard = training(train_loader, train=True)
...
save_scores = save_scores.reshape((300,2981,4))


# In[46]:


np.save('./ensemble/onevsone_train_scores.npy',save_scores)


# In[ ]:


save_hard = save_hard.reshape((300,2981,4))


# In[49]:


np.save('./ensemble/onevsone_train_hard.npy', save_hard)
...
save_scores, save_hard = training(val_loader, train=False)


# In[38]:


stacked_scores.shape


# In[39]:


np.save('./ensemble/onevsone_val_scores.npy',stacked_scores)
...
np.save('./ensemble/onevsone_val_hard.npy', stacked_hard)
...
for i, (data, label) in enumerate(test_loader):
      # forward
      data = dim1_to_dim3(data)
      count_label = onevsone_model_hard(data)  
...
np.save('./ensemble/onevsall_train_scores.npy',save_scores)
...
np.save('./ensemble/onevsall_train_hard.npy',save_hard)
...
np.save('./ensemble/onevsall_val_scores.npy',save_scores)
...
np.save('./ensemble/onevsall_val_hard.npy', save_hard)
...
def training(loader, model, train=True):
    for epoch in range(300):
        pred_label = []
        for i, (idx, data, label) in enumerate(loader):  
              # forward
            ...
            scores = torch.softmax(outputs, dim=1).cpu().data.numpy()  
            if i == 0:
                print(idx)
                stacked_scores = scores
            if i > 0:
            ...
            predict = np.argmax(outputs.cpu().data.numpy(), axis=1) 
            for i in predict:
                pred_label.append(i)
            stacked_hard = torch.tensor(np.eye(4)[pred_label]) 
        if epoch == 0:
...
save_scores, save_hard = training(train_loader, flat_model, train=True)
...
np.save('./ensemble/flat_train_scores.npy',save_scores)
...
np.save('./ensemble/flat_train_hard.npy',save_hard)


# In[41]:


save_scores_val, save_hard_val = training(val_loader, flat_model, train=False)


# In[43]:


save_scores_val = save_scores_val.reshape((300,745,4))


# In[45]:


np.save('./ensemble/flat_val_scores.npy',save_scores_val)


# In[47]:


save_hard_val = save_hard_val.reshape((300,745,4))


# In[48]:


np.save('./ensemble/flat_val_hard.npy', save_hard_val)
    ...
    for epoch in range(300):
        ...
        for i, (idx, data, label) in enumerate(loader):  
            ...
            if train == True:
                data = augmentation(data)  
            data = dim1_to_dim3(data)
            outputs1 = hirearchical_model1(data.to(device))
            cn_others = torch.softmax(outputs1, dim=1)
            outputs2 = hirearchical_model2(data.to(device))
            mci_ad = torch.softmax(outputs2, dim=1)
            outputs3 = hirearchical_model3(data.to(device))
            emci_lmci = torch.softmax(outputs3, dim=1)
...
cn_val = np.load("./weights/hierarchical/300epochs/ensemble_hierarchical/cn_external_val.npy")
emci_val = np.load("./weights/hierarchical/300epochs/ensemble_hierarchical/emci_external_val.npy")
lmci_val = np.load("./weights/hierarchical/300epochs/ensemble_hierarchical/lmci_external_val.npy")
ad_val = np.load("./weights/hierarchical/300epochs/ensemble_hierarchical/ad_external_val.npy")
hierarchical_val = hierarchical_model(cn_val, emci_val, lmci_val, ad_val)
hierarchical_val = np.eye(4)[hierarchical_val]
hierarchical_val = torch.from_numpy(hierarchical_val)
...
ovo_train = np.load("./ensemble/onevsone_train_hard.npy")
ovo_train = torch.from_numpy(ovo_train)
print(ovo_train.shape)
ovo_val = np.load("./ensemble/onevsone_val_hard.npy")
ovo_val = torch.from_numpy(ovo_val)
print(ovo_val.shape)
ovo_test = np.load("./ensemble/onevsone_test_hard.npy")
...
ova_train = np.load("./ensemble/onevsall_train_hard.npy")
ova_train = torch.from_numpy(ova_train)
print(ova_train.shape)
ova_val = np.load("./ensemble/onevsall_val_hard.npy")
ova_val = torch.from_numpy(ova_val)
print(ova_val.shape)
ova_test = np.load("./ensemble/onevsall_test_hard.npy")
...
flat_train = np.load("./ensemble/flat_train_hard.npy")
flat_train = torch.from_numpy(flat_train)
print(flat_train.shape)
flat_val = np.load("./ensemble/flat_val_hard.npy")
flat_val = torch.from_numpy(flat_val)
print(flat_val.shape)
flat_test = np.load("./ensemble/flat_test_hard.npy")
...
cn_others = np.load("./weights/hierarchical/300epochs/ensemble_hierarchical/cn_others_hard.npy")
cn_others = torch.from_numpy(cn_others)
mci_ad = np.load("./weights/hierarchical/300epochs/ensemble_hierarchical/mci_ad_hard.npy")
mci_ad = torch.from_numpy(mci_ad)
emci_lmci = np.load("./weights/hierarchical/300epochs/ensemble_hierarchical/emci_lmci_hard.npy")
...
optimizer = optim.Adam(stacking_model.parameters(), lr = 5e-5)
...
for epoch in range(num_epochs):
    ...
    if val_acc/len(val_image) > max_acc:
        ...
        torch.save(stacking_model.state_dict(), './ensemble/stacking_hard.pkl')
...
test_loader = DataLoader(TensorDataset(torch.tensor(hierarchical_test).float(), ovo_test.float(), ova_test.float(), flat_test.float(), test_label.long()), batch_size=batch_size, shuffle = False)


# In[69]:


stacking_model.load_state_dict(torch.load('./ensemble/stacking_hard.pkl'))
pred_label_pre = []
test_acc = 0.0
stacking_model.eval()
