...
#         self.train = train
    def __getitem__(self,index):
        age = self.df.loc[:,["Age"]].iloc[index].values
        if age <= 75:
            age_tensor = torch.FloatTensor([0])
        else:
            age_tensor = torch.FloatTensor([1])
...
#         Y = torch.tensor(Y)
        return age_tensor, image_tensor, label_tensor
    def __len__(self):
...
class Classifier_model(nn.Module):
    def __init__(self):
        ...
        self.fc_male =  nn.Sequential(nn.Linear(num_ftrs, 64),
                                    nn.BatchNorm1d(64),
                                    nn.SELU(),
                                    nn.Dropout(0.2),
                                    nn.Linear(64, 4) 
                                    )
        self.fc_female =  nn.Sequential(nn.Linear(num_ftrs, 64),
                            nn.BatchNorm1d(64),
                            nn.SELU(),
                            nn.Dropout(0.2),
                            nn.Linear(64, 4) 
                            )
    def forward(self, x, age): # text = [batch size,sent_length]
  
        logits = x
        m = torch.FloatTensor([1]).to(device)-age
        f = age
...
#     criterion = nn.CrossEntropyLoss()
    ...
    PATH = './mmse/weight_age_learningcurves_300.pkl'
    ...
    #     scheduler = optim.lr_scheduler.StepLR(optimizer=optimizer, step_size=5, gamma=0.9)
        model.train()
        classifier.train()
        for i, (age, data, label) in enumerate(train_loader):
          ...
          outputs = classifier(outputs.to(device), age.to(device)) 
        ...
        with torch.no_grad():
          val_loss = 0.0
          val_acc = 0.0
          for i, (age, data, label) in enumerate(val_loader):
        ...
        if val_acc/len(val_image) > max_acc:
            ...
            torch.save(classifier.state_dict(), './mmse/weight_age_learningcurves_classifier_300.pkl')
        ...
        if epoch == 149:  
            torch.save(model.state_dict(), './mmse/weight_age_learningcurves_150.pkl')
            torch.save(classifier.state_dict(), './mmse/weight_age_learningcurves_classifier_150.pkl')
...
pd.DataFrame(history).to_csv("./mmse/history_age_learningcurves.csv")


# In[41]:


# history1 = pd.read_csv("./mmse/history.csv")
# history2 = pd.read_csv("./mmse/history_2.csv")
...
plt.plot(train_acc_resnet18_pre[:148], label='train')
plt.plot(val_acc_resnet18_pre[:148], label='val')
...
classifier.load_state_dict(torch.load('./mmse/weight_age_learningcurves_classifier_300.pkl'))
...
model.eval()
classifier.eval()
for i, (age, data, label) in enumerate(test_loader):
