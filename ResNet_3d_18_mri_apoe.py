...
#         self.train = train
    def __getitem__(self,index):
        apoe = self.df.loc[:,["APOE"]].iloc[index].values
        onehot_apoe = [0] * 3
        onehot_apoe[int(apoe)] = 1
        apoe_tensor = torch.FloatTensor(onehot_apoe)
...
#         Y = torch.tensor(Y)
        return apoe_tensor, image_tensor, label_tensor
    def __len__(self):
...
class Classifier_model(nn.Module):
    ...
    def forward(self, x, apoe): # text = [batch size,sent_length]
  
        logits = x
        apoe_0 = apoe[:,0].reshape(apoe.shape[0], 1)
        apoe_1 = apoe[:,1].reshape(apoe.shape[0], 1)
        apoe_2 = apoe[:,2].reshape(apoe.shape[0], 1)
        logits = self.Flatten(logits)
        logits = apoe_0 * self.fc_0(logits) + apoe_1 * self.fc_1(logits) + apoe_2 * self.fc_2(logits)
        return logits
...
#     criterion = nn.CrossEntropyLoss()
    ...
    PATH = './mmse/weight_apoe_learningcurves_300.pkl'
    model.load_state_dict(torch.load('./weights/weight_4class_noswap_fc_affine_learningcurves_300.pkl'))
    ...
    #     scheduler = optim.lr_scheduler.StepLR(optimizer=optimizer, step_size=5, gamma=0.9)
        ...
        if val_acc/len(val_image) > max_acc:
            ...
            torch.save(classifier.state_dict(), './mmse/weight_apoe_learningcurves_classifier_300.pkl')
        ...
        if epoch == 149:  
            torch.save(model.state_dict(), './mmse/weight_apoe_learningcurves_150.pkl')
            torch.save(classifier.state_dict(), './mmse/weight_apoe_learningcurves_classifier_150.pkl')
...
pd.DataFrame(history).to_csv("./mmse/history_apoe_learningcurves.csv")
...
classifier.load_state_dict(torch.load('./mmse/300epochs/weight_apoe_learningcurves_classifier_300.pkl'))
