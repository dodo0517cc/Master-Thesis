...
#MCI_image = np.load("D:/范崇淵_碩論_2/data/MCI_seg_voxel.npy")
#MCI_image = MCI_image[:,10:90,5:115,10:110]
...
#AD_image = np.load("D:/范崇淵_碩論_2/data/AD_seg_voxel.npy")
...
#             image, _ = dataset[idx]
            ax.ravel()[i].imshow(image[50], cmap='gray')
...
#           for i,j in enumerate(range(46,66,2)):
#             ax.ravel()[i].imshow(data[0][j], cmap='gray')
#             ax.ravel()[i].set_axis_off()
#           plt.tight_layout()
#           plt.show()
          # forward
...
#         if epoch == 149:  
#             torch.save(model.state_dict(), './weights/weight_4class_noswap_fc_affine_learningcurves_150.pkl')
...
save = pd.DataFrame({"training_loss": loss_resnet18_pre,
                    "val_loss": val_loss_resnet18_pre,
                    "training_acc": train_acc_resnet18_pre,
...
save.to_csv('./weights/300epochs/weight_4class_noswap_fc_affine.csv')
...
PATH = './weights/300epochs/weight_4class_noswap_fc_affine_learningcurves_300.pkl'
model.load_state_dict(torch.load(PATH))
cam_sup = medcam.inject(model.cnn, output_dir='attention_maps_sup_4', backend='gcam', layer='layer4', threshold=0.1, label='best', save_maps=True)
_ = cam_sup(dim1_to_dim3(data).cuda())


# In[127]:


img = nib.load('./attention_maps_sup_4/layer4/attention_map_0_0_0.nii.gz')
img = img.get_fdata()
img = np.array(img)
print(img.shape)
image_sup = resize(img, (110,100,80))
...
figure, ax = plt.subplots(nrows = 2, ncols = 5, figsize = (12, 6))
for j in range(0,10):
    ax.ravel()[j].imshow(data[0,j+50,:,:], cmap='bone', alpha=0.7)
    ax.ravel()[j].imshow(image_sup[:,:,j+50], cmap='rainbow', alpha=0.3)
    ax.ravel()[j].set_axis_off()
    #     plt.imshow(data[0,j,:,:], cmap='bone', alpha=0.7)
    #     plt.imshow(image_sup[:,:,j], cmap='rainbow', alpha=0.3)
#         plt.colorbar()
#         plt.axis("off")
plt.tight_layout()
plt.show()
...
figure, ax = plt.subplots(nrows = 2, ncols = 5, figsize = (12, 6))
for j in range(0,10):
    ax.ravel()[j].imshow(data[0,j+45,:,:], cmap='bone', alpha=0.7)
    ax.ravel()[j].imshow(image_sup[:,:,j+45], cmap='rainbow', alpha=0.3)
    ax.ravel()[j].set_axis_off()
    #     plt.imshow(data[0,j,:,:], cmap='bone', alpha=0.7)
    #     plt.imshow(image_sup[:,:,j], cmap='rainbow', alpha=0.3)
#         plt.colorbar()
...
# data 50  attention_maps_sup_3
figure, ax = plt.subplots(nrows = 1, ncols = 5, figsize = (12, 6))
for j in range(0,5):
    ax.ravel()[j].imshow(data[0,j+40,:,:], cmap='bone', alpha=0.7)
    ax.ravel()[j].imshow(image_sup[:,:,j+40], cmap='rainbow', alpha=0.3)
    ax.ravel()[j].set_axis_off()
    #     plt.imshow(data[0,j,:,:], cmap='bone', alpha=0.7)
    #     plt.imshow(image_sup[:,:,j], cmap='rainbow', alpha=0.3)
#         plt.colorbar()
